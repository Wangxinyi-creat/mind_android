...
import com.pj.mind.http.exception.ResultException;
import com.pj.mind.http.exception.TokenException;
    ...
    public Object requestSuccess(@NonNull HttpRequest<?> httpRequest, @NonNull Response response,
        ...
        if (result instanceof HttpData) {
            ...
            if (model.isTokenInvalidation()) {
                // 代表登录失效，需要重新登录
                throw new TokenException(AppApplication.getInstance().getString(R.string.http_token_error));
            }
...
//                LoginActivity.start(AppApplication.getInstance(), 0);
    ...
    public Throwable downloadFail(@NonNull HttpRequest<?> httpRequest, @NonNull Throwable e) {
        if (e instanceof ResponseException) {
            ...
            responseException.setMessage(String.format(mApplication.getString(R.string.http_response_error), response.code(), response.message()));
